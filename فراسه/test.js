...
async function init() {
  try {
    ...
    const thresholdData = await fetch("./thresholds_from_pdf.json").then((r) => r.json());
    thresholds = thresholdData.comparisons || [];

    const masterData = await fetch("./firasa_master_dataset.json").then((r) => r.json());
    buildTheoryMap(masterData.theory || {});

    status("جاهز. افتح الكاميرا أو ارفع صورة.");
  } catch (error) {
    console.error(error);
    status("فشل التحميل. شغّل الصفحة من خادم محلي مثل python -m http.server 8000");
  }
}
