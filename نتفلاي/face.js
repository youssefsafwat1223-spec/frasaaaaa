...
async function analyzeMultiAngle({ front, left, right }) {
  ...
  if (front) {
    const f = await analyzeImageElement(front);
    if (f) {
      ...
      // hair / hairline / widow's peak via ImageSegmenter (optional)
      if (window.Segment && f.canvas) {
        try { out.pixel.hair = await window.Segment.hairMetrics(f.canvas, f.lm); }
        catch (e) { out.notes.push("hair segmentation unavailable: " + (e.message||e)); }
      }
    }
  }
  const sides = [];
...
function frameQuality(videoEl, lm) {
